...
6- use CV as speed divider... where we don't use CV otherwise -> input bit ORed with loop bit 
7- or rungler: XOR out with input bit
...
5--- bits in AND with this one

6- wiard1: pulse selects new data or loop old back into SR

7- wiard2: pulse selects new data or inverted loop old back into SR

8- electronotes: bits of the first SR determine (via NAND) if we recycle 2nd SR, or add new bit from the first SR 

9- other SR swopovers// feedbacks - or select as pulse out one mode???// double all modes for pulse out = 128 modes?
