...
#define randi() (adcread(0)+adcread(5)) 
#define CELLLEN 16
...
void thread_push(u8 data) {
  if (m_stack_pos<14) // STACK_SIZEE-2
	{
...
void runhodgenet(unsigned char* cellies){
  ...
  for (x=0;x<MAX_SAM2; x++){
    ...
    place=x-cellies[4]-1;
  if (cellies[place]==cellies[0]) numill++; if (cellies[place]>0) numinf++;  
  sum+=cellies[place];
...
void main(void)
{
  unsigned char cpu, plague, instruction, instructionp,oldone;
  unsigned int counter=0;
  unsigned char *cells=xxx; 
  unsigned int speed, cpuspeed,plaguespeed;
  unsigned char tmp;
  ...
  unsigned char (*instructionsetfirst[])(unsigned char* cells, unsigned char IP) = {outpp,finc,fdec,fincm,fdecm,fin1,fin2,fin3,fin4,outp,plus,minus,bitshift1,bitshift2,bitshift3,branch,jump,infect,store,writeknob,writesamp,skip,direction,die}; // 24 instructions

  unsigned char (*instructionsetplague[])(unsigned char* cells, unsigned char IP) = {writeknob,writesamp, ploutp, plenclose, plinfect, pldie, plwalk}; // 7 

  unsigned char (*instructionsetbf[])(unsigned char* cells, unsigned char IP) = {bfinc,bfdec,bfincm,bfdecm,bfoutp,bfin,bfbrac1,bfbrac2}; // 8

  unsigned char (*instructionsetSIR[])(unsigned char* cells, unsigned char IP) = {SIRoutp,SIRincif,SIRdieif,SIRrecif,SIRinfif}; // 5

  unsigned char (*instructionsetredcode[])(unsigned char* cells, unsigned char IP) = {rdmov,rdadd,rdsub,rdjmp,rdjmz,rdjmg,rddjz,rddat,rdcmp,rdoutp}; // 10

  unsigned char (*instructionsetbiota[])(unsigned char* cells, unsigned char IP) = {btempty,btoutp,btstraight,btbackup,btturn,btunturn,btg,btclear,btdup}; // 9
  ...
  void (*plag[])(unsigned char* cells) = {mutate,SIR,hodge,cel,life,inc,shift,chunk,runwire,runhodge,runfire,runkrum,runhodgenet,runlife,runcel,runcel};//TODO
  ...
  while(1){
	...
	speed=(adcread(1)+adcread(3))<<2; // TESTING! - speed of both slowing!TODO!
	//	plaguespeed=speed;//>>4; // 4 bits=16;
	//	cpuspeed=speed;//&15; // should be logarithmic and longer
	...
	//        if ((counter%(1+cpuspeed))==0){ // ??? This is where we do speed!
	  ...
	  case 0:
	    ...
	    instructionp=(*instructionsetfirst[instruction%24]) (cells, instructionp); // mistake before as was instruction%INSTLEN in last instance
	  ...
	  case 2:
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetbf[instruction%7]) (cells, instructionp);
	    insdir=dir;
	    break;
	  case 3:
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetSIR[instruction%5]) (cells, instructionp);
	    insdir=dir;
	    break;
	  case 4:
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetredcode[instruction%10]) (cells, instructionp); 
	    insdir=dir;
	    break;
	  case 5:
	    instruction=cells[instructionp];
	  ...
	  case 7:	    //la biota
	    instruction=cells[instructionp];
	    instructionp=(*instructionsetbiota[instruction%9]) (cells, instructionp); 
	...
	case EQU: if (thread_stack_count(2)) thread_push(thread_pop()==thread_pop()); break;
	...
	case POP: if (thread_stack_count(1)) machine_poke(machine_p88k(instructionp++),thread_pop()); break;
	case POPI: if (thread_stack_count(1)) machine_poke(machine_p88k(machine_p88k(instructionp++)),thread_pop()); break;

	case ADD: if (thread_stack_count(2)) thread_push(thread_pop()+thread_pop());instructionp++; break;
    ...
    case ROR: if (thread_stack_count(2)) thread_push(thread_pop()>>(machine_p88k(instructionp++)&7)); break;
    case ROL: if (thread_stack_count(2)) thread_push(thread_pop()<<(machine_p88k(instructionp++)&7)); break;
    case PIP: 
    {
	...
	case INP:
	  ...
	  case 9: // worm
	    ...
	    //      if (instructionp>((->start<<1)+(->wrap<<1))) instructionp=->start<<1;
      instr=machine_p88k(instructionp);
      switch(instr%13){
      case 0:
	  machine_poke(machine_p88k(instructionp),randi());      
	break;
	...
	break;
      case 8:
	if (machine_p88k(instructionp+(m_reg8bit2*2))==0) instructionp+=machine_p88k(instructionp+m_reg8bit1);
	break;
      case 9:
	...
	break;
	  ...
	  else modifier=cells[oldone];
	  /// invert or?
	  	  OCR0A=255-modifier;
	  //	  OCR0A=255;
    
	//    if ((counter%plaguespeed)==0){ //was instructionp%step
            (*plag[plague])(cells);
	    //	    }
  }
  }
