...
- also extra mode where we use pulse to gate if we add in the input so
  ...
  or another signal out (maybe this on LF side). use it as a toggled gate/tap TESTES

* 26/2+

- we can also use independent LFSR to clock own LFSR

- we can make a grid of uses of: CV, input clock pulse, clock pulse, SR type // and parameters such as gating, length etc. (= maybe as manual or just to classify or for new SR module)
