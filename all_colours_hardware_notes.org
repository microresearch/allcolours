...
- test PWM range with 1v steps
- test SR timing as log and if we can go higher on prescalar/period with longer speedhh etc.
- can we work in 128 modes with split of linear/exp and how to do this fast (can we handle 128 modes?)
- do we have PWM DAC follow LOG array
- test PWM DAC spacings (how could these also be variable in one mode perhaps?)
- do all modes make sense - what alternative modes there could be which take advantage of internal access to each SR bit?

(eg. differing taps, shifting taps arrays)...
