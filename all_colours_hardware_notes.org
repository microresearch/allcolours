...
for PWM modes we can maybe combine 0 and 1 (in case there are no clock pulses in)
...
also what we can keep for next one - eg. arrays of spacings, one sr as clock for other (would need to be as function pointer maybe - try this out)

* 9/1

- re-testing earlier comparator vs. newest and now stick with newer as less offset issues DONE

- TODO: fix on modes, any mode differences across each and alt taps stuff
