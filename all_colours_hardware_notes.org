...
2- what is the range for SR timing - it should also be logarithnmic but not 1v/oct
...
- thinking to norm noiself_out to pulself1_in1 and noisehf_out to pulsehf1_in1 but we need to test this -> test with all modes but why not?

do we have DAC in mode (for PWM out) - would be in pulse modes
...
TEST THIS! otherwise all ready to go - but perhaps finish and test software until we order...
