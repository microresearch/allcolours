...
- top of each filter range is kind of arbitrary - seems C8 on HF, LF we don't know
...
- testing interpolation - fixed overflow but need to test on HARDWARE
- fixed and working on hardware but needed to reduce speed of interrupt in main.c
- maybe 12k or 15k on HF filter (testing now with 15k)

* 2/6/2020++

- implementing divide down on HF side - question of modes such as 20,21 to test which already do divide downs, watch for lengthbith! // to test

- TODO: DAC modes:

 interpol 32-47 (CV) - DONE, TESTED but we can have different ranges for some...

 spacings 48-63 (pulse) - DONE/TO_TEST note that some of these have special DAC out modes already (which - these are 58-63 on HF side) TESTED

- test all pulses OUT and dividers on HF side - DONE


- fixed pulse divider on mode 20 and 21 - do this also for low side when we check thatDONE
- fixed lengthbith so makes sense now -   do this also for low side when we check thatDONE

- finish divide down on LF side and test allDONE

- interpol and spacings for LF side and test all

- changed mode 38 on both sides

* 4/2/2020

- finished all DAC modes on both sides - to test all in practice

- on low side we also have different ranges???

- new exp. mode -> so we take DAC from other SR and use this as speed! frees up CV and PULSE... mode 65 - but where shall we put this on each side - to replace?

lose 25 on LF side - as this just repeats and then we can place as mode 31 but we need to add as CV mode DONE

and on HF side what to replace? but we want as 15 so is part of CV

or as mode 1 so we can find it!
...
These are in fromzoom_2020 on intenso HD: copied to recordings here in AC as first:
...
low 166/163 with ext env in

- fromzoomoct30? 

no ext 000, 3,4,5,6,7,8,13,14,15/16,9extmod**,10lowextmod, 50-hi-speech in, 54-no cv, 55low

- octnovdec2020 - latest (keep all on zoom)// octnovdecstillonzoom

0017 - hf with one external source in (DUSG) mode 57
0018 - as above mode 58
0019 - as above +1 extra modulation source mode 61
0020//21 - as above mode 62
0022 - as above mode 63
