...
1- pwm follows DAC from SR (not same on LF as DAC bits ?what this means?>) -> speed for DAC style updates is from speed cv
...
*2- pwm follows DAC with speed CV or pulses controlling spacing depending on pulse/sr mode (or controlling range/=bits of DAC maybe in pulse modes case!)*
...
- re-testing earlier comparator vs. newest and now stick with newer as less offset issues DONE
