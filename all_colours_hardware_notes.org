...
(eg. differing taps, shifting taps arrays) - maybe we can have differing sets of modes for each side to get over constraints

*for PWM modes we can maybe combine 0 and 1 (in case there are no clock pulses in)* -> TODO and TEST how...

which gives us extra PWM mode

0- pwm follows speed cv former1- pwms follow clock pulse in for each - how we do this? speed CV as divider/multiplier 

1- pwm follows DAC from SR 

2- pwm follows DAC - different range or spacings (2 options as we use pulse/cv already for SR)
...
arrays for spacings depending on size
...
4- for PWM how do we combine clock and knob in one...(clock is multiplier of knob?) but PWM is log array
5- so extra pwm mode with DAC spacings or say different range
6- implement all modes and test
...
- do we have DAC in mode (for PWM out) - would be in pulse modes IN PROCESS
...
- any other tests/changes? - test noise as pulse in on all modes...
...
- *maybe norm pulseouts to diff audio in  instead of comparator so is full signal flow - try this idea out for different modes*



- *maybe ADC/PWM out should be upper bits as we shift in at the lowest* - how is this for other designs?
...
- mode where we invert speed of SR against PWM so does not track each other

- fixed 	//13- electronotes: CV selects which bits to set to 1 = chance of change -> with lookup array -> TESTED/DONE
