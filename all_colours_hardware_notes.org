...
(also. differing taps, shifting taps arrays) - *maybe we can have differing sets of modes for each side to get over constraints*

0- pwm follows speed cv 

1- former1- pwms follow clock pulse in for each - how we do this? speed CV as divider/multiplier of this - how to do for LOG scale 

1- pwm follows DAC from SR + pwm follows DAC - different range or spacings maybe in pulse modes - can have different PWM modes paired with pulse/cv modes?
...
- ADC in on mode 12 is at upper bits now...
...
- clean up codeDONE
///
- positions table for DACOUT spacings

- SR counter/timing range/array?
- combine clock and knob for mode 1 of PWM - PWM/DAC range
- new modes eg. shifting the array of taps (by CV or by pulse? try both), from TM - TM with probability CV (or probability from other SR!) combined with variable length
