...
newTODO:
- oscillators (as in 81 tests in adc) few bits repeating which could be pulled in on strobe from income (or from cv bits) - cycle k[reg]
- feedback as in manually cut from in to feedback(norm) pull cable
in software: feedback=dac[2]->adc cut to other dac or adc in or between 2 different dacs
- if CV>DAC - entry of new bit from [ADC, route or cycle] XOR cycle/route etc...
...
REWORKED but still need to deal with mode/adc *TODO* - test
