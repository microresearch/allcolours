...
** AC modes and inspirations: TODO
...
*check strobey, pulsins for all modeX.h modes*

** 28/1/2022

*which way round probs should be?*
...
- TODO: port newmodes from L to N, figure out calc for filter speeds
  (once we settle on speeds), define all bits, start to organise each set of modes...

*question also if LR pulses in should be XOR or OR? - OR seems better TODO/test/change*

*TODO:*
*TRY: cycling bit XOR with --> [DACout from own/other SR vs. comparator=CV/DAC/DAC+CV/CLKCNT???]*
*TODO: INTmode: probability mode where CV fixes bits (of prob) and prob is against DAC/SR onlys ???????what means* - ?prob is our LFSR - so we fix bits of this one...
...
[CV, DAC, SR] against DAC, LRC: ADC_buffer[12], CV, and combinations (ADD, XOR etc) of these
...
take from here and also *check all probability modes from newmodes.c case 17-20, check later binary routing table modes 24-26*
...
- ported in all newmodes. TODO: intmodes, 41, 47, 64, 67, 68, 69, 104, 105, 

check prob/routes 54-61, was15,14 and was11,10etc (strobes)

** 26/1/2022

*porting into Lmodes but need to resolve basics of: speed ranges and CV, probabilities, routings, basic modes for all sides*
...
can we have knob covering this and 0-10v as 1v/OCT - not sure as they are both adding...
...
- all modes have local route in to change - from strobe/bump, from CV. from DAC is already done or? - in modeN, R, L, C tested///DOE
...
- *some way to untether/leave at set speed of entry (also same for length) - run at that set speed but have dac at other speed/cv - see above*
eg. pass on speed/gshift etc, dac speed ????
...
*TODO: clkroutes for normed clock routing*
...
- *where do we put strictly Right hand modes? - eg. change routings, change where normed clks come from*
...
*DAC: 2 (is a bit muffly at long lengths - can change divy to fix this in resources.c*)
...
*we could also have mode to decide on which clks from where?*
...
change/d all modes in extramodes.c to use the dac[LFSR[w]] above rather than SR or both & 4095 but if SR is small... *CHECK*
