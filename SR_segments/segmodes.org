...
default or not: CSR DAC out->top PWMx which is normed to clock pulse for NSR
...
*NSR->LSR->CSR* // *could be default routing but then we need to see about use of SRlengthc but length is importnat if we re-circulate???*
*///->RSR->NSR*
