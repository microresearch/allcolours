...
1. shifting/SR speed <<
2. GSR copy speed (own GSR) //copy on strobe? see 37 in newmodes
...
- added binroute NOG macros to macros.h to test in modes (in all modeX.h): no movement of GSRs....
...
- *some way to untether/leave at set speed of entry (also same for length) - run at that set speed but have dac at other speed/cv*
...
*- 16x or 32x INTmodes all must use CV! we have two many*
...
*question also if LR pulses in should be XOR or OR? - OR seems better TODO*
