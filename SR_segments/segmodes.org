...
*question of strobes/toggles in probf if we use those for strobe based speed modes what happens? have probfsins for no strobe - also all of these need to use depth!*
...
- *thinking of meta-level of prob to use x or y param (eg. use DAC or CV or) -> further matrix ops*
...
- how we deal with strobe free up!? as prob (but what do we need for that? eg.)

    gate[w].matrix[9]=3<<7; // probbits as we can now use CV
    gate[w].matrix[10]=CV[w];
    gate[w].matrix[12]=8<<7; // altfuncindex // these can also be gaps!
