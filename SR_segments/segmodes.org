...
DONE: port prob_bits mode over to N

cleaning, organising, testings!

//// adding strobe modes in adc/modeN.h - *TODO: these could also be toggles*

*check all bit/route options and any implementations*
*define what CVs we can use, what prob modes, split modes for adc, dac selects, and routes/probs*
*port dacspeed modes from modeN.h*
*port from N, L to R and C maybe new modes* also multiple speed modes are in L - some for R only, some for C
*check strobey, pulsins, outs for all modeX.h modes*

*settle speeds and test new CV scheme*
