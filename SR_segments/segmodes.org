...
probzeros, probadcburst

** 3/3/2023

TODO:

- *first part of binroutes not so exciting*
- *thinking if abstracts should be in array of routebits_anystrobe_depth_notypesz ???*
- check how all esp, strobe inners handle specific use of depth in outer ????
- look at all probabilities and analysis below
- *if L50+ should be adapted for NN and then swop out c60 for c70?*
- sanity check all code, line by line - see above

TEST: 

- test extra functions to see if is worth including: from above but we need to make outers
- *check runglers on LL and RR*
