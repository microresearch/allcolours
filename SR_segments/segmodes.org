...
- simulate fracctional speeds and test

DONE.

- interpolate or not? as in just keep last value or interpolate
...
- draft new structure mode: what we need here... we will need to
  change trigger code, clkouts and will need clean up all but just
  test now...

we changed dac out but will need to change adc for pointer as some
modes change it in place...

*TODO: we also need to include stop! in speeds!!!*
...
- *TODO: INTmode: probability mode where CV fixes bits (of prob) and prob is against DAC/SR onlys*
