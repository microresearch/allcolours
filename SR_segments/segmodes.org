...
- is binroute type ever global? > yes, there is binroutetypes which is all 0, no need - TOFIX

set geogens with binroutetype to:   tmpp=gate[w].routetype; DONE - *so we have no global types, just cv/gapped*

tmpp=gate[w].routetype; is local one - where is that set - in lots of functions eg.
...
- route is count into array, type is just a type...
...
*global, local, cv 0> as a selection itself which is global, local or cv-led* // or switch between local/global -> probability
