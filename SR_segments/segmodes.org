...
- question of 16 or 18 octaves and any hardware changes...

-  *if (gate[w].changed==0) { // 1=change 0= no change and other ops in basic outer model!*

*re-check use of clk, pulsin, pulsout, strobey*

*for NN which outers do or dont use ADC- how to divide, organise*

- *strobe inners and all inners need take account of depth -check - unless is fixed array, or we use depth*

- *GeoCC ... or keep all first 16 sans gaps, fixed which might include rungler etc. - adjust when we have everything*

- *LL,, RR, NN pull rungler ops into first 16 basic modes - as in rungler with basic route in. 2x runglers - LL, RR, NN*

- *for gappedfunction we need always:*
...
- check binroute, speedroute arrays etc... to see that makes sense

maybe expand these arrays!!

- tweak: shift all strobe funcs to end of arrays and recheck all fixed speeds/funcs

- check all extents, all arrays and all functions for sanity - geogen and each before we expand across all sides...
- sanity check all code, line by line: LIST! main, caput00, adcetc, geogen, arrays, all resources, ALL!

- full tests as below
- full tests CC modes before we go further and replicate, smaller tests on runglers
- final tests and tweaks eg. orderings of functions in arrays and check these against fixed references for functions
...
matrixp/globals in RR - to test from there
12
13
14
15

- for NN, LL, RR what we have to gap in there?

** TODO this week 20+/2/2023

- finish runglers NN and LL and catalogue special functions for NN, LL, RR
- test all geoCC one by one
- finish and test matrixp ops - in progress
- ready for full tests and tweaks
...
MATRIXP: 
- we can have distorting matrices - ie. remap copy of matrixp - mask or move it with triggers and SR holes

TODO:

// globals, finalise runglers and fixed functions on each side...
// extras to fit in/maybe remove others: Zbinroutsplit, Zcyclesplit, Zbinroutsplitprob, Zbinroutsplitprob1, Zbinroutsplitprob2, Zbinroutsplitxor, Zbinroutsplitxor1//all except cycle with types - depth/no strobe
// new outer in CC: SR_geo_outer_Cplast last one held // can be like indie speed

Zlast - should be option for routes//entry into probs, same as basic cycle
...
- *we need to check use of depth in all function arrays, see about combining or simplifying arrays*
...
as prob funcn of  binroute vs altroute[count] // sans binary Zbinrouteforalt - *need to finish altroutes...*
...
// *for some functions we need IN as gate[dacfromopp[daccount][w]].dac*
