...
// but question now if we need for ROUTE as we have theroute which is only ever set by depth (so as long as we don;t always set it is ok)
...
ZzbinrouteINVbits (first test conversion)

Zzbinroutebitscycle, Zbinrout, binrout, binroutI, binroutAND0,
...
binroutor, (these not setting theroute) 
...
- look again at final abstractions - in geoC but not sure of these???
