...
- idea for 1 or 2 versions with different modes... also why generic version is silent on early/first modes - no incoming bits or loopback?
...
ADC (0-19 now):
  ...
  *TODO: new adc with x bits ADC in subjected to LFSR*

*0 should have adc in and another mode no in with pass through so all 0s still runs*

tested up to 11 (12-19 todo), fixes trigger as static
...
- DONE: added in normedd L,R,C clocks from Cspeed2 - using new count5 - seems to work// *can also have LR as DACs?? maybeDONE-TOTESTwiythINTMODES*
...
*change all modes to use the dac[LFSR[w]] above rather than SR*
