...
- LARGE notebook notes: possible to use Rmodes as global modes for:
  global routing changes, multiple DACs out/mix, potential several
  ADCs ins (in different modes). In CVmodes we would either have fixed
  modes brought in or param, bump up routes. In INTmodes we could use
  CV for modes.
