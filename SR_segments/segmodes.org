...
*TODO: maybe clean up in new version if we have to take a break - work on TOUCH/TOAD to complete*

*settle on final speeds across all and check*

RESOLVE ARCH:

*static/frozen speeds in general or not - yes but just not on lowest*
*slower speeds/cv*
...
- platform to manipulate permutations

- choose abstraction and fill in modes (check what we have - old code is easiest maybe), and then deal with details like clk, puls etc.


*BASIC DECISIONS*

key qs: CVs speed and length, ringing nature of mid route sometimes and slowest speeds for route 
- should speed be so slow as we can always use pulse for very slow speeds

interpoll is only for dac so keep to a minimum options there and mostly in modeC

** TODO

new dir/files

what are models: abstractions, mishmash here, minor/major modes,
ghost/tails, layers/hold/detach, modes initiate streams (below), all
as bits and functions/gens, 

sliding of matched functions - so there is major one and others deviate from that by wrapped value...

16x16x16x16=65536/// how we can address all global route permutations... a value for each... 4+4+4+4=16 bits for one set of global routes

*as would be nice to have global routes from SR but at present we just use arrays... - how to bridge this gap*

- co-ordinate and check lists of routes, dacfrom, speedfrom routes
- ghost routed layers trial
- 5th tail as processor, generator and what_would_have_been
- check all above: speed, clkin, pulsin etc.
- sort modes
...
R- set of global routes eg. cut all routes on strobe, sync routes, bump routes etc. // what do we have already?

not so well organised there in modeR.h:
...
g// start from scratch with basic modes and tail, modes for each side to complete and test
...
z: route in from x, spd from y + own offset // optional route in from zz- OUT
...
1binroute_/2binroutesr_/3binroutealt/4zeroes/5shared/6nos, 7trigger, 8toggle//orOR
...
- DONE;length from dac/lengthfrom also: speedfrom, routefrom, lengthfrom, logicfrom 
