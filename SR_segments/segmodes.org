...
*notes:*
- do we need if ==w in succbits
- zcopyGSR, zcopyGSR_s unused and potential indie versions
...
XX#include "exp_port.h" - cleaned up a bit but needs full check.
...
// divide into code checks, tests and if we can fit any additional code?

- *check defaults we copy into first arrays*
- *check we have parameters for probs in functions*
- look at all probabilities themselves
- sanity check all code, line by line - see above

- *any additional probs and functions?*
- *if L50, L51, L52, L53 should be adapted for NN and then swop out c60range for c70range in NN? again memory Q*

///

*UNUSED*

SR_geo_outer_Cplast - where to fit? on any side?

added in geoCC: SR_geo_inner_probdepthnodepth but we need outer to use it! -> is probf depth with routebits_nodepth_typesz 

NN: 
probzeros: SR_geo_inner_adcprobzero, SR_geo_inner_adcprobzeronod
SR_geo_inner_probadcentryxornod and all nod/no depths below - which need outers?
...
CC:
- SR_geo_inner_probdepthnodepth: anystrobe depth//routebits_no depth UNUSED
- we need also probf_anystrobe_nodepth vs routebits_depth_typesz

** 7/3/2023

*potential adc in mode - adcin as depth and all abstract choices// maybe some more simpler ones esp on NN*
*also abstract depth from dac on RR TODO 0- where to fill in -c13 in both cases!*
