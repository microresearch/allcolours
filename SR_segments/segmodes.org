...
- generic ADCg_ function to TEST but mostly keep other ADC_ ...
- *new ADC_ cases 63, 64, 65/also int65 to test, other tests in modeN.h*
...
*test: ADC-prob can also be against DAC rather than LFSR - but always CV against - other prob options in CV modes*

- *in progress: can we have one intmode with CV selecting non-param ADC input modes...*
- *TODO/test/in progress adc_66: ADC intmode various mixes of ADC incoming plus/modulo/etc/XOR CV[0]*


** 17/1/2022:

TODO: all ADC modes: prob of entry, prob of return for abstract modes, INTmodes-speedasCV/prob/comp

- *think about inverting CV for param/int modes as now 0 is top TODO! DONE - to test and do we need to invert for prob intmodes?*

*TEST dacspeed from itself?*

*TRY: cycling bit XOR with DACout from own/other SR vs. comparator=CV/DAC/DAC+CV/CLKCNT???*

*modulo LR modes to code/test*
...
- *fill in modes but we need to plan out // way these look* start drafting these -> re-look at theone modes
