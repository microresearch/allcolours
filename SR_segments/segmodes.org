...
// chnaged OUTV_ to OUTVN_ in modeN and as we have no pulse out in N!!! only in L, C and R!


- *where are we with extramodes.c and other source files?*
...
catalogue all adc modes for length...etc

// adc modes which have set bit depth, variable bit depth, restricted to <11 depth
// also check dac modes
