...
6/12/2021: fixed major problem in MACROS draftdec // if else...
...
- summarise again probability modes and all modes/cv/INT:
...
- prob of: -invert incoming bit (routed, cycle or pulse-ifwehave) or not
           -XOR incoming bits (cycle, routed, ADC, pulse-ifwehave) or not/otherlogop (XOR with?)
           -incoming bit(cycle, routed,ADC,pulse-ifwehave) or cycling bit
           -as above with inverted cycling bit
           -changes/manipulations to the ghostSR

3. binary route/prob/etc. tables

look again at binary routing tables and prob bits

4. experimental/overlaps/other functions eg. clock101010
...
- toggle entry or pass (0)... check in adc? ADD in modes
- where do we add prob of ADC entry bit - in mode itself - but this doubles up modes//how to deal with taht
...
TODO: - modes which manipulate the ghostSRs! - what those manipulations could be - freeze it, freeze its length, reverse it - but how to reverse based on length - reverse lowest srlength bits
