...
*dislocations of: ordercount of SRs in globals*
...
*- still not so resolved on local/global/set by depth - need clesr policy as in some modes is so/not - or we set ROUTE to this...*
...
- [do we need copy matrix from first modes -> matrixp or do we use gaps CHECK? // only if we use full matrixcopy]
...
new zz types added/to add in: added: zzwiardbits2I, zzwiardbitsI, zzwiardinvbitsI, zzsuccbitsprob, zbinroutfixedfixed, zzsingleroutebits, zbinroutfixed_prob1R, zbinroutfixed_prob1L 
