...
*where do we set globflag and inflag? see below*
...
*maybe pull uses of depth into all non-depth so we don't need: if (nodepth==1)*

// TODO: *use globflag where needed in geogen/exp_port - also where we set this*

// DONE/TODO: *but would be nice to locally shift on theroute - add these./change to: zbinroutmybumpSSXXX, zbinroutmybumpbitXXX, zbinroutmybumpbittXXX, zbinroutmycvD*

// *what do we do with the few no_depth ones we have? convert to internal prob??? just use as length*
...
just set a flag if we need to set route at all... (if depth is not used)? 1 means depth is used... // but what of no depth ones...
...
zpattern8bitsID, flipflopD, flipflopID, ztogglebitsshS, ztogglebitsS // 24

NO depth in above: zllfsrbits, ztogglebitsshS, ztogglebitsS // can be probbed
...
no types: zjustcycle, tailbitsInos, tailbitsnos, tailbitsI, zjusttail // justcycle we have as prob already, maybe get rid others...

no depth/types: zbinroutmybumpS, zbinroutmybumpbit, zbinroutmybumpbitt // now these are for theroute and we keep them...
...
// no depth/no types: pbitSRroutelogXXX, pbitSRroutelogxxXXX, pbitSRroutelogxxxXXX, pSR32XXX, pSRN22XXX, pSRN21XXX, pSRN15XXX, pSRNwas15XXX ???


*redo all bitsn for new types: and fill in exp_port as route/abstract/strobe etc...*
