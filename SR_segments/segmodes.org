...
- DAC rework. code in 000 // *SR_geo_outer_testDAC*

- global/local flag - how to test - by setting in a simple case (do test code) // SR_geo_outer_testglobal *TODO* // test set from outer inner geoC route *SR_geomantic_outerRglobroutetest*

- in geoC test last 71+ prob modes *SR_geo_outer_C71*

- TEST: we have logical ops: binroutorg, bitrouteorgap: *SR_geo_outer_testOR*

- TEST - prob of CV vs. global route - do we have that? set routes 0 now as prob5 and prob5_nosh: *SR_geo_outer_testprob5*

- test changes to runglers in geoC: *SR_geo_outer_C52 and SR_geo_outer_C53* // what were changes?

-  SR_geo_inner_probcycle(uint32_t w){  // TESTY *SR_geo_outer_testprobcycle*

- tests for abstracted route list *SR_geo_inner_noprobTEST*

- trial matrixp draft: *SR_geo_outer_C01matrixp* 
...
- *and this is also wrapped up dislocations of: ordercount of SRs in globals // *is there any potential for dislocation across SRs and what this could be??*
...
3. Q. as not all types use dacparam but needs to make sense together... - *so do as length too or seperate these in DAC_* TO_TEST
