...
- how to have pulsing code on DAC which is like on for x runs and then
  ...
  with very slow running other SR - how to test this? -> is 2 routes in (one as OR) - is just usual multiple routes in but needs to be AND (can be OR for inversion) - where to implement?
