...
*we need to fix clksr and gclksr spdfuncs*

- draft: scheme to hold speedfunc and values (for how long, how to hold, hold on exit/entry)

DONE to some extent - to TEST speedselcvm is 11
...
cvcount becomes part of gate[x] so each one has one and is untied from funcs - funcs could also have own count


*more flexible way of handling - see below, how there can be room for variation, also how an advance through modeR/matrices becomes more complex in terms of recursion etc*
