...
- *check new functions against depth*
...
- - new runglers, check rungler routing (see notebook) and options - clkfrom other/incoming data 

- when we decide we eliminate theroute and globflag...

- // *also we need to use routebits_anystrobe_notypesz*

*zbinrouteSRbits is rungler depth->route->bits* so can be used on left side as speedfrom
