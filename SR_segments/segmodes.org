...
DONE:3 changes to HW  - update board, output files and BOM accordingly/check production files on imac
...
** 23/3 - final code checks ALL

TESTED:ests/changes: C70, //C153 // check all speed cv and cv in*

//DONE: where we use depths and thus need set prob:

SR_geo_inner_gappedfunction
SR_geo_inner_gappedfunctionC
SR_geo_inner_str_gappedfunction

checked final mode C153

DONE- where do we set and use [12]?

setting:

NN: C72 but not in w==0. do we use it? C70,71,72.73 to check: in 70: SR_geo_inner_gappedfunction we use it even with w==0 - FIX!?
not set but used!

LL: C62 we set but do we use it C60,61,62,63 to check - used in 63: probdepth -OK!
  
RR: C62 but not in w==3. do we use it C60,61,62,63 to check -used in 63: probdepth - but not used for w==3 - OK!

// and is prob used in ouroboric functions? we are only concerned with w==0/NN NO!

** 20/3 - CODE FREEZE! EQUINOX

ONE - fixed speed issue in 153!
DONE- DONEcrash tests and code checks, checked ADCadvance and reset...
CHECKED: bug fix in pSRLLbumproute: routebits_depth_typesz 22<<6, and spdvienna4

** 19/3

- DONEre-test fixed mistake of copy to matrix in C153!
- DONEre-test C133 and general fixes to layers across ouroborics, and c150 changed

** 17/3

Final tests: 

DONEtest all ouroborics again

TESTED/done:
- added dac mode 30

dacfrom->DAC out param [14] in CC? added in C71 to test

- check ADC/DAC: match basic adc with timed dac out and check again all adc/dac code
/// 
FIXED: dacs: 21, 29, 31 have no CV control!
//ADC/in: input HW brings up/adds offset and inverts (do we invert reading)
//- test dac[0] out // we invert output


** 16/3

- trial [0]=3<<7 // no interpol on right as default

TESTED:
;left inTEST- trial CVL smooth or not in general
- check runglers on RR side: [now R52X, 53X, R60X, R62X, 63X] 
////so LL is: 41-43 50,51,52, 60
speed now at 42 in main.c so less jitter but see how that feels!
- TEST/fix???jumpiness/noisiness in some modes - speed issues - how to test (pulse code in long functions), how to fix?), dacs tested now so not that
last mode still to do - speed of move through inners... (what order
  though - match list of last modes for each side - if mode has
  changed add it but then array needs to match this??) is not on RR as that has own last mode

DONE:
measured: 110mA +12v, 20Ma -12v

//- that speed mode is quite important for dac/dacfrom control - only as interpol/non interpol - but we set at C40, N40
