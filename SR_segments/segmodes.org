...
*matrix mode which can encapsulate former modes (without running out of memory, also all the adc/dac types we had there)*

a study in recursion

recursion is a process

SRx determines route SRy to give value to determine route SRz to  ...
...
[mode] determines [new mode] - timed/triggered? return to [mode]
