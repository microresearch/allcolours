...
*would be nice to gap funcbit but then we don;t know what is done with depth - now we have depth array in gate[w] trial this...*

*if (!depth_routebits_nostrobe_notypesz[gate[w].matrix[3]>>7])*   
...
-  new runglers, check rungler routing (see notebook) and options - clkfrom other/incoming data: *zbinrouteSRbits is rungler depth->route->bits* so can be used on left side as speedfrom

- when we decide we eliminate theroute and globflag...

- // *XXXX also we need to use routebits_anystrobe_notypesz - fix NOTYPES!!! and use this somewhere!*
...
- *in progress: maybe divide depth and no depth as mix is confusing for use of depth* test new prob <24 funcs with top speed and CV as sel
...
*did it crash? how to crash test - with rand on modes but doesn't crash and is slow...*
