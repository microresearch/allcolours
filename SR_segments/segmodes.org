...
how we can break down more into tasks and not all so general
  ...
  eg. python code to generate matrices, possible generic functions and
  new recursions, functions for order, porting of functions, stack functions, 5th tail, how
  we can interfere and disarticulate matrices... and then using insights from these
  start to resolve structure eg. if we have more general geomantic
  functions and not just the geomanticxx
