...
*static/frozen speeds in general or not - yes but just not on lowest - but how to implement - implemented now as spdfracend -> speedfunc 6 // new default*
...
- different tails and how we select these... can also be fake
  strobe/flipflop say at moder speed ... mix of generators, and copies
  of SRs at full speeds... as tail is new param, and a potential route in

** xxTOP

** 12/10/2022

- *thinking of meta-level of prob to use x or y param -> further matrix ops*

from 10/8: *geomantic: gate[w].matrix[10]=(gate[dacfrom[daccount][w]].dac+CVL[w])&4095; // detach this too // or rather one new form of re-attachment - we can also use value functions here!! // another layer of attach*



TODO: 

- more probf for 16 or 32 probfs?
- how we deal with strobe free up!? as prob (but what do we need for that?)
- break up bitns into different sections eg. binroutes, but what of all those exp. modes // also general binroute types
- fix matrixp ops.
- more tails to do... what they could be as runs at full speed. consecutive bits, 
- flesh out modes
- details like strobe above
- speed of slur, see TODOs below

DONE: ADC//upto 32 ops and implement reset/other, 

NOTE:// add and other ops - only issue is if depth and in are both the same dac = how to avoid this? different dacs for adc -> eg. = incoming gate[dacIN[daccount][w]].dac

** 11/10/2022

- new geoC.h and geoN.h started to sketch modes

TODO: extra matrix for dacparDONE, start to make sense of all functions and depth
