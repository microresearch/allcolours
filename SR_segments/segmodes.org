* how to classify/generic ideas sept 2021
...
pulse/cv speed, input ADC/LFSR, output DAC/else?, type SR, type SR, type SR(3 bits of types=8), routing from   

1               1               1           1              1        1                                1 1

is already 8 bits which would be 255 modes!!! we have limit of 64 modes

unless we have other ways of thinking of modes...
...
as in we can mix use of arrays of pointers and shiftregs with names which point to these...
...
freezing is not so interesting as just locks and gives continuous tone...
...
- also odd patterns just realized unless we constrain lengths to odd numbers (which is even lengths) *TEST this - improves but maybe keep odd option*
...
routing can be array of pointers
...
if (routex==1) do this... // or pointers to entries
...
- what we do with both clock and pulse in bits in CV mode and how we
  ...
  speedCV as control for) - otherwise we have pulseouts normed to all other pulse/clock ins
