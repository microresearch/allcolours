...
- started to wrap DAC functions... so we can use CVL, CVM to select

TODO below.

- TODO: seperate out function and CV matrix//DONE

- how we have matric select by CVM when/if matrix has cvm within it - seperate matrix sets???
...
how to insert say dac/adc values into matrices eg. dactype is from CVM???

// but we do this using meta-functions which can select others eg.
// eg. gensel for bits
// to add for dac etc... -> but dac is type and not function so... has otherpar-cvdac so how do we select?
// for adc functions we also use 2x CV/ins - we need extra param for these in MATRIX -> expand...
// *or we fix functions to use CVL,CVM to select TODO*

// just for adc and for dac
...
*new routeprobs to use//can also have adcprobs with depth -> prob of cycle, prob of hold last value etc...*
...
// *how to program for lisbon - more generic?*
