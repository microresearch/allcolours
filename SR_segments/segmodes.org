...
- *re-check use of clk, pulsin, pulsout, strobe*
...
- *check: for gappedfunction we need always:*
...
TESTED- check resets are working on all sides // DONEcheck reset code
...
- check/annotate how modes use CV(x) and CVL(y)
- sanity check all: especially probabilities and w==0 of substitute functions
...
- above notes and below - what we didn't have a chance to use:
