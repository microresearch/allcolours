...
*deprecate: TODOremove all - globflag is also in all resets...*

deprecated: make sure we set IN where it might be needed - see new code in C03alt for that new flag which can also be played with*

*double check all functions, and all extents for called arrays*
...
- *add abstract index as [17] was taken...*

- *can maybe add more orderings to orderings array!*
...
--> *how do we reset matrix/matrixp to something sensible if say is all attached, by filling gaps - settle down*
--: *problem is that matrixp is null to begin with, do we need to set every op as matrixp in outers...*

eg for speed gate[wh].matrixp[0]=&gate[wh].matrix[0];

*crash in 122/setvar...: strobe is routebits_nostrobe_depth_typesz//extent 7 =32; speed is speedfromstrobe_nodepth[gate[w].matrix[15]>>8]*
...
*and all globals to look at again*
...
- *might need to genericise all the prob functions to fit in memory now and then re-test*

- *test all strobes in geoCC again as changed [3]->17 for strobe func*
...
- *check: do we still use in flag:   if (gate[w].in) gate[w].matrix[5]=gate[dacfrom[daccount][w]].dac;   else gate[w].matrix[5]=LFSR__[w];*
