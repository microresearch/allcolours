...
*baisc model, test basic geogens inc. ported//classic SRs, ADC and DAC, arrange, layout of inner and outer - new pointer arch, tails, interpol/strobe etc, recursions/gaps/attach/detachments, final tests/tweaks*
...
*arch: sets of outers calling inners, can we fix different models (eg. matrixp copies?), 16 strobe modes with lots of ops, mobile gaps, add different binroutes, lengthchanging modes, bituse below, more adc modes*

filling in structural gaps: we added new local binroute but could also be different types of binroute // local binroute is ONLY routing so NOT a different type except in one case
...
*or we just use incoming bit://binroute in//depthroutein//local route in where and when we need it*
...
- *we can also break down CVs to smaller chunks and have smaller selections eg. of just routing modes*
