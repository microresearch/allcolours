...
-2. what is in main loop?
-1. how fast are other interrupts
0. are we running as fast as we can 180MHz or so...
1. not execute all DACs but just use SR (and what of shifting?)
2. different structure so all in one go w0-3 and slow down all...
3. faster ADC., different structure
4. generally faster codes, ifs, swicthes, leave out some things
5. speed of generic code
...
- confirming that for striaght through length (above 11) doesn't change anything (also length of left pass through) - becomes one long sr
...
*change all modes to use the dac[LFSR[w]] above rather than SR or both & 4095 but is SR is small...*
