...
- break up bitns into different sections eg. binroutes, but what of all those exp. modes // also general binroute types to organise
...
** 26/27/28 - binroute section and start to fill out geoC
...
- *probability in regular bitmodes is now vs. in and we pass LFSR to matrix[5] <(LFSR_[wh]&4095)// remember - but then we need to handle these - different sets...*

/////
TODO:

- bring down bitops to 64 where needed and try that

- re-test etc modes

- re-check rungler functionality: remind of that!
