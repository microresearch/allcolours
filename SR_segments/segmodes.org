...
- *Q: if we OR with binary in depth/theroute or just in binroute* - global Q!!!
...
*check/TODO: fix probs which use strobe in strobe speeds, also ones which use IN - *to fix all probs for gaps in geoC* // which of probfsins use IN: spdfrac2,3,4,spdfracdac3, zosc2bitsI, compbits, spdvienna - always set an IN// CHECK TODO
check/fix probs in geoC
...
*fix depth and route gaps: depth=gate[wh].theroute; for gapped routes eg. CV[4]=gate[wh].theroute<<8; but only when we know it is about a route*
...
fixed route means global route

1. binroute fixed [dac/adc type]//reset
2. length and dacparam 
3. Q. as not all types use dacparam but needs to make sense together... - so do as length too?
4. depth as type/type is now set as IN:   gate[w].theroute=depth; fixed route
5. depth as route only. type IN gapped
...
10. select routebitsfortypes, gapped depth and type // depth can be route or param
11. depth cv
...
- catalogue missing and extra geogens - below...
...
// what we need to add from below:

- new route modes to add: binroutorg, binroutgap(nod), bitrouteorgap(nod) - and add versions of these but maybe test first ... note that we do divide into ones which use depth/no depth! eg. routebitsnod

- binroutfixed_prob5 and binroutfixed_prob5_nosh (to binroutprobs)...

- new NZ ones we added today...
...
- TEST: we have logical ops: how does that work eg. route 8OR1=8+1 (and if both 8 then just 8 so adds route, AND = neither, also new gapped theroute, also expand those but how to add
