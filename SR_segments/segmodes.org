...
- DONE/test: *seperate matrix for abstract ops* - replace what was route with 18 and maybe DONE:take care of all theroute and globflags already
// but we also need abstract CV on 19! to addDONE - test all

// *taking care of globflags but we will need to re-test all - esp binroutorg // all speeds, routes etc.. logic check all functions one by one eg. depth=depth>>8 was missed/other shifts, all extents catalogued*
