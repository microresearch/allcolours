...
#include "geoLL.h"
#include "geoRR.h"
#include "exp_port.h" - cleaned up a bit but needs full check
...
- *check parameters for probs in functions*
- *any additional probs and functions?*
- look at all probabilities themselves
- *if L50, L51, L52, L53 should be adapted for NN and then swop out c60range for c70range in NN?*
- sanity check all code, line by line - see above
- we need more memory if we add/change too much
...
-TEST:Now in SN for matrixp alts: dep//SR_geo_inner_probdepthdepthSS: probdepth//notypesz // where?
