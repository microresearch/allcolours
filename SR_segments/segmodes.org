...
TODO: test latest new codes eg.  loopback par (cvmod), bitsmod, outside modes such as delay (add delayoutDONE),



- extracting function bodies with emacs macro - but leaves ints DONE
- added loopback -> par so cv selects cv
...
2.5- start to organise matrices...

3- move on to recursion and major modes
