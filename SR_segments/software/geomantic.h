...
//1speed 
// now with depth/CV
uint32_t (*speedfromsd[32])(uint32_t depth, uint32_t in, uint32_t wh)={strobe, spdfrac2, spdfrac3, spdfrac, holdlspdfrac, strobe, ztogglebits, ones, clksr, clksrG}; // one interp, next not - see interp below... interp is extracted...
//0 is strobe
...
uint32_t *CVlist[4][16]={
  {&nulll, &gate[0].dac, &gate[1].dac, &gate[2].dac, &gate[3].dac, &CV[0], &CVL[0], &CVM[0], &ADCin, &Gshift_[0], &clksr_[0], &param[0]},
  {&nulll, &gate[0].dac, &gate[1].dac, &gate[2].dac, &gate[3].dac, &CV[1], &CVL[1], &CVM[1], &ADCin, &Gshift_[1], &clksr_[1], &param[1]},
  {&nulll, &gate[0].dac, &gate[1].dac, &gate[2].dac, &gate[3].dac, &CV[2], &CVL[2], &CVM[2], &ADCin, &Gshift_[2], &clksr_[2], &param[2]},
...
void SR_geomanticxx(uint8_t w){  // for split func/cv
  ...
  if ((*speedfromsd[gate[w].func[spdfunccnt][fspeed]])(*CVlist[w][gate[w].cv[cvcount][cvspeed]],*CVlist[w][gate[w].cv[cvcount][cvspeedmod]], w)){ // speedfunc
    ...
    if (gate[w].cv[cvcount][cvadcIN]==8){ // real ADC
    ADCgeneric2; // input into shared one..
    }
