...
// TODO: less routes in or why route in is noisy...
...
void SR_geo_inner_norouteadcN(uint32_t w){  // new template
  ...
  if (interpfromnostrobe[gate[w].matrix[0]>>7]){ 
    ...
    else     bitn=(*abstractbitsz[gate[w].matrix[20]>>extent_abstractbits])(gate[w].matrix[5], gate[w].matrix[4], w); 
    BITN_AND_OUTV_; 
    new_data(val,w);
    }
}
...
// select abstract/adc + route // now try with speed select...
