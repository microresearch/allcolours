...
static inline uint32_t NZbinrout_probXY(uint32_t depth, uint32_t in, uint32_t w){  // was RD
  ...
  if ((depth>>4)<(LFSR_[w])) tmp=tmp^15;
      //  if (depth<(LFSR__[w]>>4)) tmp=tmp^15;
  tmpp=gate[w].routetype;
  ROUTETYPE_;
  return bitn;
}
...
static inline void flip(void){ //at full speed is just average ... so is boring
  // what could be inspired by tails/caput tail mouth
  uint32_t w=8;
  GSHIFTRED_;
  //  gate[8].flip^=1;//+=gate[3].flip;
  gate[8].shift_+=gate[0].flip;
...
static inline uint32_t zbinrouteSRbits(uint32_t depth, uint32_t in, uint32_t w){   // depth as routesel... SR itself, no GSR 
  ...
  //  depth=15-depth;
  if (depth==0) { // SR5 is 8th which is outside these bits 
    bt = (gate[8].Gshare_>>SRlength[8]) & 0x01; 
  } else
    {
  for (uint32_t x=0;x<4;x++){
  if (depth&0x01){
...
static inline uint32_t spdfracdac3self(uint32_t depth, uint32_t in, uint32_t w){ // depth is offset, in is constraint -- and speed from speedfrom
  uint32_t bt=0;
  float speed;
  int32_t tmp;
  if (in==0) in=1;
...
static inline uint32_t strobespdfracP_(uint32_t depth, uint32_t in, uint32_t w){ // for speed now with dac/interpol pulled out
  static uint32_t bt=0;
  float speed;
  gate[w].strobed=1;
  speed=gate[w].logspeed[depth>>2]; // 12 bits to 10 bits
...
static inline uint32_t zinvprobbits(uint32_t depth, uint32_t in, uint32_t w){   // PROBability mode - other way round
  uint32_t bt=0;
  if ((depth>>4)>(LFSR__[w]>>4)) bt=1;
  //  if (depth<8) depth=0; // limit
  //  if ((depth)>(((LFSR__[w]>>4)))) bt=1;
  return bt;
}
...
static inline uint32_t zprobbitsxortoggle(uint32_t depth, uint32_t in, uint32_t w){   // PROBability mode xor strobe - can be more ops
  static uint32_t bt=0;
    gate[w].strobed=1;
  if (gate[w].trigger) bt=bt^1;
  //  if ((depth>>4)<(LFSR__[w]>>4)) bt^=1; // variations
  if ((depth>>4)<(LFSR__[w]>>4)) bt^=1;
  return bt;
}
...
static inline uint32_t Zbinroutsplitprob1(uint32_t depth, uint32_t in, uint32_t w){   // 
  ...
  if (spdfracx(depth, in, w)){
    bob=(gate[w].Gshift_[w]>>SRlength[w]) & 0x01;
  }
  bbb=bob;
  }
  return bbb;
