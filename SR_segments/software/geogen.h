...
static inline uint32_t ZzbinrouteINVbits(uint32_t depth, uint32_t in, uint32_t w){   // depth as routesel... shared bits now NO LONGER USES DEPTH
  ...
  //  depth=binroute[count][w]|binary[w]; // or we can just pass in depth
...
static inline uint32_t zzsuccbitsprob(uint32_t depth, uint32_t in, uint32_t w){   // we route from each sr in turn
  // include itself or not
  uint32_t bitn=0, bitrr, tmpp;
  static uint8_t x=0;
  if (x==w) x++;
...
static inline uint32_t zbinroutfixedfixed(uint32_t depth, uint32_t in, uint32_t w){   // fixed binroute from count //global
  uint32_t bitrr, tmp, tmpp, x, bitn=0;
  if (gate[w].globflag) tmp=binroute[count][w]|binary[w]; else tmp=gate[w].theroute; // was tmp=binroute[count][w]|binary[w];
  tmpp=gate[w].routetype;
  ROUTETYPE_;
...
static inline uint32_t binroutorgap(uint32_t depth, uint32_t in, uint32_t w){  // no depth
  uint32_t bt=0, bitrr;
...
static inline uint32_t ztogglebits(uint32_t depth, uint32_t in, uint32_t w){   // toggle - no depth
...
static inline uint32_t ztogglebitssh(uint32_t depth, uint32_t in, uint32_t w){   // toggle - no depth
...
static inline uint32_t zllfsrbits(uint32_t depth, uint32_t in, uint32_t w){ // no depth
  uint32_t bt;
  static uint32_t k;
