...
// 6/6/2022: pulling below into experiment.h for trials
...
spd - function/bits, dacasspdfrom, cv, fixed as last
length - daclengthfrom, CVL, fixed
[adc - if there is adc in - null adc also is any we don;t have eg. 777
bit/route - function/bits, binroute/binroutetype
prob/logic/other

generic modes - all use lists of functions referenced from global/local counts
expanded modes eg. split speeds, all others: we need to put what we can under global control eg. binroute

so we have tmp=binroute[count][w]|binary[w] as routes and bitfunc[bitcount][w] as list of functions ???

but bitfunc needs to have access to binroute[count][w] as well...
...
// so each mode is defined by: speedfunc, lengthfunc, bitfunc which are indexes into list of functions
// functions can just return cv etc...

// grids as follows

//1speed
uint32_t (*speedfroms[8])(uint8_t wh)={sadcfrac, sdacfrac, sstrobe, sstrobeint, sbinroute, sprob, sprobdac, sclksr}; // sycycle also... // expand for dacs etc...
...
//2length
uint32_t (*lengthfroms[8])(uint8_t wh)={lcvl}; 
...
uint32_t (*bitfroms[8])(uint8_t wh)={}; // add in funcs which use also binroutetypecount
...
// bitfroms is an array of functions which can include different types of binroute etc...
// generic and have versions of this...
void SR_geomantic(uint8_t w){ 
  HEADNADA;
  gate[w].dactype=whichdac[dactypecnt][w];
  if (speedfroms[speedfunc[spdfunccnt][w]](w)){ // speedfunc
    SRlength_[w]=lengthfroms[lengthfunc[lengthfunccnt][w]]; // lengthfunc
    GSHIFT_;  // replace with types of gshift - gshiftfuncs...
    // CORE 
    bitn=ADC_(w,whichadc[adctypecnt][w],bitn,dacfrom[daccount][w],param[w], &gate[w].shift_); // as adc functions or as bitfuncs????
    bitn^=bitfroms[bitfunc[bitfunccnt][w]];// includes logic - would be nice deal with whole SR but pulsins etc. comes later // bitfunc
    // ENDCORE
...
// that count can also be an index into functions - 2 different versions below

void SRcount_template0(uint8_t w){
  HEADNADA; 
  if (speedfroms[slist[spdcount][w]](w)){ // speedfunc
    SRlength_[w]=lengthfroms[llist[lengthcount][w]]; // lengthfunc
    GSHIFT_;  
    // CORE 
    bitn=bitfroms[blist[count][w]];// includes logic and types of binroute - would be nice deal with whole SR but pulsins etc. comes later // bitfunc
    // ENDCORE
    BITN_AND_OUTV_; // pulsin is in there
    new_data(val,w);
  }    
...
void SRcount_template1(uint8_t w){
  HEADNADA; 
  speedf__=logspeed[gate[speedfrom[spdcount][w]].dac>>2]; // spdcount
  CVOPENDAC;
  if(gate[w].last_time<gate[w].int_time)      {
    gate[w].shift_=gate[w].shift_<<1; 
    tmp=binroute[count][w];
    switch(blist[binroutetypecount][w]){
    case 0:
    BINROUTEstrip_;
    break;
    case 1:
...
void SRx_x(uint8_t w, uint32_t strobey, uint32_t detachlen, uint32_t  detachspeed, uint32_t interpoll, uint32_t (*innerfunc)(uint8_t w)){
  ...
  if (w==2 || speedf_[w]!=LOWEST){ // ignored if we don't have the stop... and if we are modec - insert in function
    // interpol or not
