...
** rethinking hardware and software...
...
// re-write below based on our decisions...

*- what we can also do with PWM like pulsing it!*
...
base PCB:
- remove one set of jacks (which)
- all conditioning of ADC signals is on panel ... 
- start on kicad

- wait aisler//test panel... one section as sketched... no gnd plane on back...

*lose VCA and have CTRL output also/instead - or ghost output*
...
- working now (not before) with adjustment to voltage out of first stage/PWM - set this gain in the first PWM amplifier...
- TEST: finger in of other signals/split levels, what is optimal size of gnd around???
...
- potential mismatch of rec and play lodges - how they need to match, how *one is more like a bend, mirror or ghost of the other/s*
- DONE: abstract out more inside VIA functions, to test and apply to RPmodes

** 23/7 +

TODO:
- resolve swops, micromodes, resets new schemes... 
- SORTout of sync on playback - shifts over time and with changes in speed...*
- DONEto revert to just delcntt: seems better but unsure of overlap: to *TEST*
- describe full simple model of action and presses, implement and test
- how we can move towards abstraction which makes all ideas more possible ... --> *abstraction that allows for recursion and embellishment with some simplicity*

// notebook notes:

- how we can have states per layer? that state only changes master/active layer - after swop. to rethink swop
- problem of overlay in reclodge (say after reset): 
---> should reclodge do any overlays? Q for multiple zones, 
---> how fast can we reset one position - both layers =0 

//

*micromode now where P button would be?*

** 22/7+

eg. in RP we can maybe use V also as controlling
eg. also in NR, contract the zone using CTRL (shorter and shorter) - but there is no feedback...

- how recorded CTRL can control play or rec zones....

TODO:

- DONEtest all infra now:

- implement new toggles and basic modes and test/play:

Q: is if it is press/release of toggle while modifier is held... or press/release of modifier while toggle is held - or both

trial for rec/play - *now we just use REC button to toggle state*

R=NR
P=RP // and if there is no R zones what do we do...? do not enter?

how we deal with swops under each state? NR=swop R, RP=swop P/or both?

toggle only: swop/reset that section - TODO:to fill in resets (not full one) = toggle and NO modifier, no CTRL...

toggle+modifier: state(NR/RP), mode, micromode [mode/micromode->advance]
toggle+CTRL: params on zones -> P or R zones

[CTRL+modifier: ???  CTRL is local to 2 sections - can also be micromode/mode advance or params on zones...]

modifier only: global

////

- genericise functionality - and find framework for ops
- how could test.c print parallel plays/recs as lines?

** 20/7

- *New schemes:*

- that with NR mode we need to pay more attention to length of
  loop/zone and what we record, multiple zones or not, zones dictated
  by gesture or by silence (that means by Voltage or by CTRL)...// *check that function in rec again...*

eg. also how we do silences outside reclodge - how to genericise all...

what voltage is output, what is recorded, what is played...

//
functions:
Always rec -> zones (and we need overlays there) // no play 
RP/overlay // again see what kinds of overlay we have below // play and rec (so live overlay, recorded overlay)

P: live overlay and overlay of zones on each other
R: overlay value on what is there (see reclodges)
RP: what overlay is recorded/bounced from/to

define bounce: //bouncefrom - we get currentval from zone x and overlay on reclayer

//

CTRL and feedback of recorded CTRL!

//

- eg. add new islands/zones
- switch between rec/loop to zones? //simultaneous or not... ????what?

** 19/7

*TO FIX: major issue with overlap as we add lodges...*

TEST!Solved for extend lodges - mode 0

1-addlodges: NONoverlap mess/fine, TODO:test overlap

*overlay writes over itself as start->maxrec* - which can be an option

but we want to start over - overlay in addlodges is odd in all ways
once we overlay as we have overlapping playlodges...

2-silence
3-nosilence

//Solve now for add lodges - in progress - and all silences - we need to record length from now on and just wrap rec and play..
...
delay of next delcntt does not include our offset as that is calculated in

- DONE/TEST -  add loop point for rec - play will loop with no delay...*
- *TO try: null delay in rec = simultaneous record to zones*

Scheme:
- manipulate zone parameters eg. add/copy zones
- play/rec zones

TODO:
- new touch scheme and dual modes only
- TODO: currentval/done and bouncefrom, attachment function of rec and for play, PLAN!

-->  uint32_t bouncefrom; // lowest bit can be layer 0,1, rest is zone?

bouncefrom layer 0, zone 0


** 15/7

- test all infrastructure, recs:
//- fixed
- why is delcntt copy so critical
- *QQ: with if (f[d].rl[0].ind==1) we don't need function in rec but could we need it???*
...
- DONETEST recording over 21 seconds // now 18 seconds
