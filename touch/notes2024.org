...
Basic mode to implement: (each mode is defined by ADC and freeze behaviour)

NADA: ADC->mode // freeze as freeze (in other modes as fake swop? invert etc) - invert is take value from top, but what is "fake swop"?
REC: ADC->overlay to other tape // freeze as swop main tape... // further REC adds to main tape...
PLAY: ADC->speed // freeze is swop
REC+PLAY: ADC->speed/bounc/rec to other // freeze is swop
...
- moons from the top
- which of four ADC has hardest press, how we could interpret 4 presses as binary/geomantic figure!?
...
- re-test all VCA, noise and voltages
- new finger tests - Kaffe: triggers, length of trigger, new voltage?

- new speed mode to test

- condense and print older/newer notes/here

- define basic HW and timing, filters below
- define minor modes 
