...
PLAY: ADC->speed // freeze is swop // do we add to list on swop/play-play?
...
Nada: add global from top, invert voltage, ghosts and mirrors - but does these stick around? yes
Rec:
Play:
RP:
...
R: only reset R or not on entry into R + N options // or leave that reset...
...
*so now sensitivity if for newADC and for Voltage -> manual*


** 1/2/2024

- embarked on new test code with all basics to test
- TODO: *implement and test activation by way of extended TOGGLE/FREEZE*
