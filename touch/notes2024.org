...
All infrastructure to test and catalogue...

TODO:

DONE/TEST - implement micromodes per layer (and per state - but states have different main layers:
DONE/TEST - implement basic RP mode: just write new values to existing lodges, playback existing lodges (do we check that there are)...*
