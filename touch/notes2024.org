...
TODO:
- DONE;added-leaving functions// *to test - also re-test R_addlodges*
- *record silence gap as offset*
- *each succesive rec mode adds one zone*
...
*2 definitetasks: define zones, play from/record to those zones (can be seperate or at same time)*
...
! of how much "feedback" to our actions we have:
eg. RP: 
...
// *that there are 3 different kinds of RP: overlay on top of looping R // bounce playback onto layers (feedback)* - are these defined as major modes now?

*what are modes now: N?, R has no feedback...???*
