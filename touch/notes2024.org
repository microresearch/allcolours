...
*TO FIX: xmajor issue with overlap as we add lodges...*

1. Solve for extend lodges - mode 0
2. Solve for add lodges

- issue in rec/addlodges as start advances too far... 
- case if we have zone 0 which extends to overlap then real length is MAXREC
- but if we add zone which goes past overlap its length is not MAXREC...

we need to record length from now on and just wrap rec and play..
...
- *with if (f[d].rl[0].ind==1) we don't need function in rec but could we need it???*
...
- new touch scheme
- TODO: currentval and bouncefrom, attachment function of rec and for play, PLAN!
