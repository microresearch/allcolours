...
Toggle for each position: operation on position according to mode, longer press (1sec+) toggles position as active/inactive.
...
DONE:- to port latest playlist into 2024.c and trial there -> playing of playlist // recording of playlist
TO TEST!
...
- *first mode - can add to playlist for later modes. next mode just plays that, doesn't add*
...
- *re. kaffe - why is freeze working better than mode toggle? - need to test kaffe again with slower toggle*
...
- *TODO: major/minor reset:  perhaps have now soft and hard resets - eg. soft resets rec and play tapes but doesn't change other mode stuff...*
