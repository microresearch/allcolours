...
NADA: GEOMANTIC: ADC->mode // freeze as freeze (in other modes as fake swop? invert etc) - invert is take value from top, but what is "fake swop"?
REC: ADC->overlay to other tape // freeze as swop main tape... // further REC adds sections to main tape...
PLAY: ADC->speed // freeze is swop
REC+PLAY: ADC->speed/bounce/rec to other // freeze is swop // what does voltage do (overlay same tape if >// minormodes here)
...
- define minor modes 
- define and test basic mode and add/catalogue all modes
...
Nada: add global from top, invert voltage, ghosts and mirrors
Rec:
Play:
R+P:

new: ghosts and mirrors for all

*** Speeds:

- sync speeds to top newADC or not
- new logfast_stop and logspeed_stop
- 2 types speed: logspeed and logfast  

*** etc?

- to reset or not
- sensitivity?
- extend overlay past end of recording
- detach/unsync (how that happens - as it needs to unsync to a time - so ie. re-sync to say Freeze) - options for what freeze toggle does in certain modes

detach would be to stop/start play/rec head... with freeze/toggle...
...
- what minormodes we have so far? in [0]?
	...
	//R+P: overlay
	overoverlay=((overminor>>1)&3); // types of overlay
...
- Geomantic code select with newADC only in NADA: so we have 4 sections for that...
- Activated groups have R, and P. Do we have Mode on X or X is SEL, and where do we put modes? 

- ORDERED: aisler /// moon figures for panel!
...
eg. newADC runs through/scrolls through list of segments as index into

- NADA - micromodes sets use of touch/voltage (eg. add global from top, inversiosn, ghosts and mirrors) which stays through all...
