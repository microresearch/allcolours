...
note that lower crescents have voltage ...
...
so layer can be same as CTRL also above... and layer/ctrl can also be a gesture which determines operations...
...
*5/6 - new scheme to remove R,P,RP and N - to use active and inactive!*

- DONEtesting test4.c and new 002.c - added delay code for zones/lodges and fixed overlap code TODO: reset
each layer has its own zone list - added linkage in code... but linkage defaults to zero= *fix*


TODO:
- redo all timings and main loop for simultaneousness (main = 24*8=192)
- simulate and test adding to playlist
- remove R, P, N, RP from code
- replace buttons as mode, micromode and reset... (also recheck active/inactive code) - mode is per each active channel so all can have different mpdes
- record and stop on zero/silence to implement and test
- how playlist can also have overlaps?

** 3/6 + to try to finish// notes overview...

*How to implement new scheme below: sequences of manipulations, other layer as XXX, overlap playlists, access other sections, groupings and sync*
...
- *does playlist per layer make sense or - then we need another lost to indicate time of swopping - also q of overlay of 2 layers in playback?*
