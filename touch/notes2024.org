...
- *question of passivity/brainstrom approaches eg. in NADA, CTRL triggers recording of a zone,,, still Q of modes!???*
...
- new RP mode to work on
- adding new zones in rec to trial: is that R mode - when we have a
...
- how much layer voltage can feedback onto layer selection itself - recursion...
...
- timing full tests with cammack/myself...
- trial new overlays above

- list what we want to achieve with modes - eg. repeating islands, feedback and recursion
- test zero ADC + increase max number of zones - where is set? 
- trial list of play zones and abstract out layer access (eg. pass layer list to play)
- consider new simultaneous model above...
- list overlays and CTRL/layer functions and test/play these
- draft second mode with zones - start rec on touch - or this as first mode: plain rec vs. zone list?
...
TODO:
- simulate and test adding to playlist//now we have new play zones to test
