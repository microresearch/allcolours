...
prev:
- how to have different speeds for each zone (but zones are rec anyway?)
...
?list of: real time eg. 00. in, out, length if any, layer/section, linkage

TODO: simplest op for rec is to write value to: section/layer/array=location using whose_dacc_counter
