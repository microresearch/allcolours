...
- of levels of feedback (via. playback) and dying out in feedback (with some overlayRP)

* diary notes

TODO/general:

2 options/micromodes for R and P, RP to implement:
...
_ *DONE/to test: check delcnt and realend - with other rec opts - with silence doesn't work - so only delayed/unsynced one - simulate*
