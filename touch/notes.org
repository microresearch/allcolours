...
how many major modes? say 16= 8 global, 8 mixed/local // minor modes= 8 bits/255 so far
...
*so where we get speed from (eg. speed for each voltage comes from VCA of that section)?*

///
TODO:
looping rec/play???
tap rec/rec: start/end
tap play/play: start/end

freeze has differing functions and speed modifier too...
...
// playing with all modules: overlay is most important to have... but then we need to clearly reset...
...
do minormodes vary depending on state?

States:         // minormodes

- norec/no play: sensitivity / type of freeze
- rec          : ^^         / ^^              / rec speed / resets
- play         : ^^         / ^^              / overlay/freeze/speed/resets
- rec+play:    : ^^         / ^^              / rec speed / overlay/freeze/speed/resets

- rec and play: as special case/major mode... special case of play or or rec?
