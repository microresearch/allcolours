...
DONE13/14/15-/-10 - TESTED: DRAFTED - still to test... 

///

6 and 7...

TODO: 3- [81] - or it could also be the original and we make all full overlays in top bits, freeze/unfreeze toggles between the two for each!!*
// so same as 5 but we keep length the same and do togglings
...
unsync... (reverts to nada for rec or play and to play for overlay???)

so this leaves: 

- change rec/play speeds - but in a volatile way...
- mirrorings
- islands???
