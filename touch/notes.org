...
*question still of minormode for sensitivity, also no sensitivity of VCAs to test again*

** Modes in progress // to be filled out with minormodes and tested:

// what FREEZE (call it TOGGLE maybe) can do in overlaid in each case, rather than freeze??? or keep freeze in first 2 modes
// we also need to test [6] voltage in all cases and other voltages all at once...
...
2: mode with global speed recording  //minors TO TEST in full

IN progress with no minors, so just proof of concept:
...
- we can have VCA as speed or voltage as speed...

// recordings overlay just/only for VCA or not, also what toggle/freeze does in overlaid case - ignore speed overlay?

see below:

5- [79]? - overlay one repeating section with different overlays *REWORK mode 79, we can store original loop there(where?), and expand overlay in top bits ???*
...
13/10 - DRAFTED - still to test... 14/10 - tested - fine but we need to be able to change length of that loop!
...
1- [79]? - overlay one repeating section with different overlays *REWORK mode 79, we can store original loop there(where?), and expand overlay in top bits ???*
2- [80] - islands????
...
3++ - as above with ongoing overlay in top bits, and TOGGLE clears that overlay - so we can do it again...

// below....

DONE4+ local speed recording on voltages 
4++ local detach and others
