...
*define global modes, global/local and local modes and see all below - maybe another way through LUNAR*

- any influence from lunar, geomantics?

how many major modes? say 16= 8 global, 8 mixed/local

minor modes= 7 bits/127 so far
...
*looped rec/overlay: or rec starts that loop/play, and stops, and play just plays back/no recorded overlay, how do we reset?start rec again*
...
no play/no rec: freezer behaviour, speeds
play: overlays, freezer behaviour, resets, overdubs, start and end, mirrors, speeds
rec: speed/slew, overlays, freezer behaviour, resets, overdubs, start and end, mirrors, speeds
...
** new major modes (and mods where we can)
GLOBAL:
1: basic/simple mode with all mods: 0
2: overlay on each rec: 1 // or is that minor?
3: ????slower recording speeds - longer, steppy: 2 - test again // unsure as is behaviour change...

global/local4: detach play: 15

5: looped rec/overlay: 9/10
...
11: record speed on freeze top - always play back speed and re-record on freeze
12: record speed: how differs from 11?CHECK? - this one is overlap - does it make more sense than 11? 
...
default bits in mode 0 // 9/6/2023:

12-speeds: add, add from below, speed as real or recorded
3--overlay:modulo/peak in playback and also see freeze behaviour below
45-resets: play, rec
6-freeze on [6] freezes speed, freezes value // can also be other seperators of speed/value - freeze dependent
7-in playback: freeze stops playback, freeze holds overlap

*8-extra bit could be: record overlay or not? or keep as major mode...*

1 2 4 8 16 32 64 // 128
1 2 3 4 5  6  7 // 8
We use NONE speed as major mode option -> 5 bits=32

19/6 added in theory: freeze behaviours (also in playback with other modifiers): 

- freeze as holding and we overlay on top (then we use type of overlay or do we need to double that)
- freeze as just holding with no overlay (default we have now)

- freeze only releases on reset (held for x time) - with or without overlay...

also type of overlay - we can just add a new value or we can add difference - if it is greater than so just goes up and is released
...
- 10M still works for triggers (also tested on touch/toggle)... but 10M/3.3M/2M raises voltage/base offset...we can go 1M - try 1.6M = 400mV offset, 1.2M is negligable..
// we can try 1.3M

so we have 1.6M on lowest voltage and 10M on mode trigger - to test all with many fingers...
