...
- add protection, re-test PCB - also with new processor if we choose, BOM and order
...
TESTED- case 7: voltage as sectional speed only on playback - differs for different adcs TESTING?
...
- started as case 9:TODOloop mode: - mode which is always recording in loop (and type of overlay - start with modulus), and rec/play buttons (length of holding down) set start and end points?
also as variable speed mode,,,


///

TODO- how we can record global changes in speed... tap of freeze on top speed one...
...
- full open and close of VCA - test on lowest one -> R26 OUT1TOMIX -> 27k tested, no bleed and full voltage (changed on PCB/schematic - but lots of errors to fix there)
