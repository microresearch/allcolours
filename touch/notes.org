...
////charactersistics/parameters to work through:

rec/play: speed, length//start-end, counter, mirrors, overlays, freezers doX, resets of params...

*looped rec/overlay: or rec starts that loop/play, and stops, and play just plays back/no overlay*
...
GLOBAL:
1: basic/simple mode with all mods
2: overlay on each rec 
3: slower recording speeds - longer, steppy

4:

5: looped rec/overlay
6: record speed seperately
7:
8; mirror - all follow one voltage[lowest=4] and overlay own...

GLOBAL/LOCAL:
1: length of start/end (global/local tho!)
2: freeze overdubs/new 
3: freeze detachs playback
