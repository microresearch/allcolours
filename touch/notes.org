...
0: basic mode with overlay and speed control on [6]
1: basic mode with overlay and no speed control
testing//in progress: 2: mode with global speed recording

3. local speeds - could be local triggering and all take [6] as voltage, or local voltage
...
9++ record general speed, overlay and general speed (where did we store general speed before? in [6]???) [11/12 ]
...
TETSING: TODO: mode 2: generic speed from [6] recorded where (for all of them - speed overlay?) // q below of overlay...

- we need record in play on freeze as before - or always record in play

Q if we should record in overlay or not as gets messy - NOT - so overlay stays same as 0 - *no as needs to read back speeds! or that is an option for major or minor*  !

--> to port to 0 mode with all mods
