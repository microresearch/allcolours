...
uint32_t R_addlodgesx(uint32_t d, uint32_t V_options, uint32_t R_options){ // each succesive rec mode adds one zone
  ...
  f[d].rl[f[d].masterLR].ind=1;
    ...
    if (f[d].rl[f[d].masterLR].num_lodges==1){ // first lodge
      f[d].rl[f[d].masterLR].lodges[0].cnt=0;
      f[d].rl[f[d].masterLR].lodges[0].delcnt=0;
      f[d].pl[f[d].masterLR].lodges[0].delcntt=0;
      f[d].rl[f[d].masterLR].lodges[0].start=0;
      f[d].rl[f[d].masterLR].lodges[0].offset=0;
      f[d].rl[f[d].masterLR].lodges[0].delay=0; // looping???
      f[d].rl[f[d].masterLR].lodges[0].end=MAXREC; 
...
uint32_t playlodge(float speed1, float speed2, uint32_t d, uint32_t P_options){ // pass in the p/layers
  ...
  uint32_t sample=0;
  // tape 0 lowerP
  for (x=0;x<f[d].pl[f[d].masterLP].num_lodges;x++){
    if (f[d].pl[f[d].masterLP].lodges[x].delcntt>=f[d].pl[f[d].masterLP].lodges[x].offset && (f[d].pl[f[d].masterLP].lodges[x].delcntt<=(f[d].pl[f[d].masterLP].lodges[x].offset+f[d].pl[f[d].masterLP].lodges[x].realend-f[d].pl[f[d].masterLP].lodges[x].start))) {     
      tmpx=f[d].pl[f[d].masterLP].lodges[x].cntt+f[d].pl[f[d].masterLP].lodges[x].start;
      f[d].pl[f[d].masterLP].lodges[x].cntt+=speed1;

       if (tmpx>(f[d].pl[f[d].masterLP].lodges[x].realend)) { // now we want final delay before we start again
	tmpxx=tmpx-f[d].pl[f[d].masterLP].lodges[x].realend;
	tmpx=f[d].pl[f[d].masterLP].lodges[x].start;
	f[d].pl[f[d].masterLP].lodges[x].cntt=tmpxx;
	}
      if (tmpx>=MAXREC) tmpx=MAXREC-1;
      //      sample=overlayx(recordings[d][tmpx]&BOTS, sample, (P_options>>5)&3); // deal with overlap
      printf("play: x %d tmpx %d cntt %f delcntt %f del0 %f end %d delay %d\n", x, tmpx, f[d].pl[f[d].masterLP].lodges[x].cntt, f[d].pl[f[d].masterLP].lodges[x].delcntt, f[d].pl[f[d].masterLP].lodges[0].delcntt, f[d].pl[f[d].masterLP].lodges[x].realend, f[d].pl[f[d].masterLP].lodges[x].delay);
	    
    }
    //    else printf("xxx delcnt0 %f \n", f[d].pl[f[d].masterLP].lodges[0].delcntt);
    if (f[d].pl[f[d].masterLP].lodges[x].delcntt>=(f[d].pl[f[d].masterLP].lodges[x].offset+f[d].pl[f[d].masterLP].lodges[x].realend+f[d].pl[f[d].masterLP].lodges[x].delay-f[d].pl[f[d].masterLP].lodges[x].start)) f[d].pl[f[d].masterLP].lodges[x].delcntt=0;
    else f[d].pl[f[d].masterLP].lodges[x].delcntt+=speed1;
  }
  return sample;  
}
...
uint32_t R_addlodges_silence(uint32_t d, uint32_t V_options, uint32_t R_options, uint32_t val){ // add zones on press and offset is silence
  ...
  if (tmp>LOWVAL) { // start recording
    ...
    if (f[d].rl[f[d].masterLR].num_lodges==1){ // first lodge
      ...
      printf("sil %d\n",f[d].rl[f[d].masterLR].lodges[tmpx].sil);
      //      f[d].rl[f[d].masterLR].lodges[tmpx].offset=(f[d].rl[f[d].masterLR].lodges[tmpx-1].realend)+(f[d].rl[f[d].masterLR].lodges[tmpx-1].offset)+f[d].rl[f[d].masterLR].rcnt; // add silence if any...
      f[d].rl[f[d].masterLR].lodges[tmpx].delay=0; // to come later
      f[d].rl[f[d].masterLR].lodges[tmpx].end=MAXREC; // to come later
      f[d].rl[f[d].masterLR].lodges[tmpx-1].flag=1; // stop the last one TODO: remember to reset
    }
  ...
  */
  ...
  R_addlodges_silence(0, 0, 0, 0);
    ...
    for (x=0;x<f[0].rl[f[0].masterLR].num_lodges;x++){
      printf("no: %d offset: %d start: %d end: %d delay: %d flag %d fulldel %d \n", x, f[0].rl[f[0].masterLR].lodges[x].offset, f[0].rl[f[0].masterLR].lodges[x].start, f[0].rl[f[0].masterLR].lodges[x].end, f[0].rl[f[0].masterLR].lodges[x].delay,   f[0].rl[0].lodges[x].flag, (f[0].rl[f[0].masterLP].lodges[x].offset+f[0].rl[f[0].masterLP].lodges[x].realend+f[0].rl[f[0].masterLP].lodges[x].delay-f[0].rl[f[0].masterLP].lodges[x].start));
  }
...
*/
    ...
    // copy in and play...
        ...
        f[0].pl[f[0].masterLP].num_lodges=f[0].rl[f[0].masterLP].num_lodges;
    for (x=0;x<f[0].rl[f[0].masterLP].num_lodges;x++){
      f[0].pl[f[0].masterLR].lodges[x].delcntt=f[0].rl[f[0].masterLR].lodges[x].delcntt;
      f[0].pl[f[0].masterLR].lodges[x].offset=f[0].rl[f[0].masterLR].lodges[x].offset;
      f[0].pl[f[0].masterLR].lodges[x].delay=f[0].rl[f[0].masterLR].lodges[x].delay;
      f[0].pl[f[0].masterLR].lodges[x].start=f[0].rl[f[0].masterLR].lodges[x].start;
      f[0].pl[f[0].masterLR].lodges[x].realend=f[0].rl[f[0].masterLR].lodges[x].realend;
    }
